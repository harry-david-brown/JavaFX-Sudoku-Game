...
public class GameManager extends Application {
	...
	private RadioButton radioButtonDesign;
	...
	/**
	 * Changes the CSS ids of a specific game board element according to its
	 * original state
	 * 
	 * @param array
	 *            an Array of CSS ids
	 * @param i
	 *            the location of the button in the game board
	 */
	...
	private void reset() {
		...
		// Creates a new game board for the player
		game.clear();
		game.generateBoard();
		game.generatePlayer();
			...
			for (int j = t; j < t + 20; j += 9, temp++) {
				...
				for (int k = 0; k < 3; k++) {
						...
						boardText.get(pos).setOnAction(e -> {
							...
							if (game.checkBoard(board)) {
								...
								Alert alert = new Alert(AlertType.NONE,
										"You just completed the game board in " + count/1000
					...
					boardText.get(pos).setOnMouseEntered(e -> {
						if (!game.checkBoard(board) && value != 0) {
	 ...
	 * appearance in the player's game board
	 */
	...
	/**
	 * Creates a timer and displays it in the title bar
	 */
	private void startTimer() {
		start = Calendar.getInstance().getTime();
		timeline = new Timeline(new KeyFrame(Duration.seconds(1), e -> {
			count = Calendar.getInstance().getTime().getTime() - start.getTime();
			stage.setTitle(
					"game - Time: " + String.valueOf(TimeUnit.SECONDS.convert(count, TimeUnit.MILLISECONDS)));
		}));
	...
	public void start(Stage primaryStage) {
		...
		hbox.getChildren().addAll(newGame, clear, initializeRadioButtons());
	...
	private HBox initializeRadioButtons() {
		//initialize radio buttons
		radioButtonDesign = new RadioButton("Design");
		radioButtonPlay = new RadioButton("Play");

		//add radio buttons to toggle group		
		ToggleGroup radioButtonsGroup = new ToggleGroup();
		radioButtonDesign.setToggleGroup(radioButtonsGroup);
		radioButtonPlay.setToggleGroup(radioButtonsGroup);
				
		//add radio buttons to pane
		HBox radioButtonsGroupPane = new HBox();
		radioButtonsGroupPane.getChildren().addAll(radioButtonDesign, radioButtonPlay);
		radioButtonsGroupPane.setAlignment(Pos.CENTER);
		radioButtonsGroupPane.setSpacing(15);
		
		return radioButtonsGroupPane;
	}
